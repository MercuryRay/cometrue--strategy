...
export default function Header() {
  ...
  return (
    <header
      className={`sticky top-0 z-50 backdrop-blur-xl border-b transition-[background-color,border-color,box-shadow] duration-300 ${
        scrolled
          ? 'bg-white/95 border-neutral-200/80 shadow-[0_1px_24px_rgba(23,23,23,0.07)]'
      ...
      <div className="max-w-[980px] mx-auto px-6 h-14 flex items-center justify-between">
        ...
        >
          {desktopNavLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`nav-link hover:text-neutral-900 transition ${
        ...
        <div className="flex items-center gap-3">
          <a
            ...
            target="_blank"
            rel="noopener"
            className="btn-sheen hidden sm:inline-flex items-center gap-1.5 bg-line text-white text-[15px] font-bold px-5 py-2 rounded-full hover:bg-line-hover hover:-translate-y-0.5 hover:shadow-[0_10px_24px_-10px_rgba(6,199,85,0.55)] transition-all duration-300"
          >
